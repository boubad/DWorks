...
namespace CPPTestProject
{
	...
	///////////////////////////////////////////
	TEST_CLASS(UnitTestCritElem)
	{
	public:
		//
		TEST_METHOD(TestArrange2Mat)
		{
			...
			MatArrangeType oArrange(&oIndivs);
			size_t nClasses = 5;
			oArrange.arrange(nClasses);
			const size_t nx = oArrange.size();
			Assert::AreEqual(nClasses, nx);
			std::wstringstream os;
			for (size_t i = 0; i < nx; ++i) {
				const ClusterType *pCluster = oArrange.cluster_at(i);
				Assert::IsNotNull(pCluster);
				os << *pCluster;
				os << std::endl;
			}// i
			 /////////////////////
